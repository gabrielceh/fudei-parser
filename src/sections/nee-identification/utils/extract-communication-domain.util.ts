...
export const extractCommunicationDomain = (text: string): CommunicationDomain => {
  ...
  // 🔹 Nivel Expresivo
  result.expressive = bool(match(/Nivel Expresivo:\s*(SI|NO)/i, text));
  // 🔹 Nivel Receptivo
  result.receptive = bool(match(/Nivel Receptivo:\s*(SI|NO)/i, text));
